...
- **Large-queue strategy:** the mock API is seeded with 503 requests (3 hand-written fixtures + 500 generated by `mocks/generateRequests.ts`, picked from realistic per-category title templates so search/filter demos still return sensible matches). Filtering/searching is done client-side with a single `Array.filter()` pass (`filterRequests.ts`) over the already-fetched list — at ~500 items this is sub-millisecond, so it isn't the bottleneck. The request list itself is **not virtualized**: each `RequestCard` is a small number of DOM nodes (a title + three badges), and React's key-based reconciliation keeps re-renders on filter/search changes cheap at this scale — confirmed by testing in the browser (DevTools Network tab shows one `/requests` fetch on load, no repeated fetches while typing, since filtering never re-hits the API; typing and changing filters updated the list with no visible lag). If the dataset grew an order of magnitude larger, or rows became heavier, the next step would be list virtualization (e.g. `@tanstack/react-virtual`) or server-side filtering/pagination — reaching for that now would be solving a problem the numbers don't actually show at this scale.
